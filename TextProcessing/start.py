...
data_inputan = sys.argv
fileCsvInput = data_inputan[1]

def load_data():
    data = pd.read_csv(fileCsvInput, delimiter =';',encoding= 'unicode_escape') #Memanggil data dari file cvs
    return data
...
# print(df['hasil_steeming'])
for data in df['hasil_steeming']:
    print(data)